...
- **Debian with systemd as PID 1, or Alpine with OpenRC.** The init system is
  not incidental here: the six services are systemd units on one and OpenRC
...
- **Tools you can extend.** Each tool is one `.py` file in `/opt/boa/tools/`.
  Drop a new one in and it appears in the interface.
...
- **Answer them from Telegram.** Pick an agent with /agents, or reply to a
  message one sent you, and what you write reaches it, starts a run and comes
  back answered on your phone. The whole exchange is in that agent's
...
- **Fourteen languages.** German, English (UK and US), Spanish (Spain and
  ...
  your agents which language to answer in, and what the Telegram bot says.
...
4. Asks whether to serve the application on 11080/11443 behind an HAProxy on
   ...
   again nor quietly changes it.
5. Creates one agent: `manager`, the orchestrator. Everything else is an
   example you pick from when you press **+** - `webnavigator`, `oswatcher`,
   `mailwatcher` and others, one Markdown file each in `backend/agents/examples/`. The
   empty agent is the first choice offered; the examples come under it.
6. Installs and starts six services: systemd units on Debian, OpenRC
...
`supervise-daemon`, and what they print goes to the system log:
...
- **No agent runs as root, ever.** Only `boa-exec` does, and it accepts a
  closed list of ten operations over a Unix socket. There is no "run this
  command" among them.
