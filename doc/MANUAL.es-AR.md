...
- **Alpine con OpenRC.** Los siete servicios son scripts de OpenRC supervisados
  ...
  instalador agrega `openrc` él mismo cuando la máquina no lo tiene.
...
Abrí `https://tu-servidor/` y entrá con el correo que le diste al instalador y
la contraseña que generó. Si no la tenés:
...
**Cerrar sesión** abre una confirmación en el centro de la pantalla. Confirmá
para salir, o elegí **Cancelar** o apretá Escape para mantener la sesión abierta.

Sólo hay una cuenta. Cambiá su contraseña en **Ajustes → Cuenta**.
...
es justo para lo que se cambia una contraseña cuando creés que alguien
más tiene una.

La primera vez que abrás la aplicación después de una actualización te va
a pedir iniciar sesión otra vez. Las sesiones anteriores a la
actualización no llevan constancia de cuándo se concedieron, y a «no lo
puedo saber» se responde preguntando.
...
suspensivos: pasá el mouse por encima para leerlo entero. El cuadro
...
apenas le enviás un mensaje de chat o apretás **Ejecutar ahora**, y se para
cuando la ejecución termina, venga de vos, de su programación o de una tarjeta
que venció. El contorno se comprueba cada pocos segundos, así que puede ir un
momento por detrás.

Abajo del todo, dos puntos indican si los servicios de fondo están en marcha.
Si alguno está rojo, los agentes no se ejecutarán: mirá
[Cuando algo no funciona](#cuando-algo-no-funciona).

El botón **+** crea un agente.

## El agente con el que empezás
...
**ejemplos**, que se proponen al apretar **+**.

## Los agentes de ejemplo

Al apretar **+** se te pregunta por dónde empezar:

El **agente vacío** encabeza la lista, porque es la única respuesta que siempre
sirve y cada ejemplo es un atajo hacia él. Los ejemplos van debajo, por orden
alfabético:
...
| **web-navigator** | Maneja su propio navegador para hacer en un sitio lo que le pidas: buscar, entrar con tu sesión, llenar un formulario, ir pasando páginas y contarte lo que encontró. |

`web-navigator` es el único sin programación horaria: trabaja cuando se lo pedís
y el pedido ES la tarea. Es también el que muestra para qué está instalado el
navegador: entra en sitios, aprieta botones y llena formularios, mientras que
los demás leen páginas. No escribe contraseñas, no compra nada ni aprieta un
...
Son puntos de partida. Cambiá el prompt, las herramientas y la programación, y
tené en cuenta que varios dicen en su propio prompt qué necesitan que completes
...
`/opt/boa/webapp/backend/agents/examples/`. Dejar un archivo ahí agrega un ejemplo a la
lista; no hay nada más que tocar. Si preferís que no te pregunte, desactivá
...
- **Arreglar sí suele necesitarlos, y no los tiene.** Su prompt le dice que
  ...
  1,2G» está bien más que un intento fallido.

Si querés que algo se arregle solo, poné ese comando en el crontab de root. Un
...
Apretá **+** y dale un nombre. Eso crea, en el servidor:
...
o aprietes **Ejecutar ahora**.

## Hablar con un agente

Hacé clic en un agente de la barra lateral y aparece su chat. Escribí lo que
querés que haga y apretá Intro; Mayúsculas+Intro hace un salto de línea.
...
techo existe para frenar una programación desatendida, no para frenarte a vos
...
La conversación no es sólo lo que escribiste vos. Cuando llega la hora de una de
...
muestra todo lo que se le pidió, por vos o por otro agente, y lo que hizo con
eso. Ver [Cuándo se ejecuta una tarjeta](#cuándo-se-ejecuta-una-tarjeta).
...
En **LLMs**, elegí un proveedor. La lista es corta a propósito: ofrece los
...
ahí, así que no se ofrece. En cuanto guardás su clave, aparece.
...
El campo del modelo sugiere los que sirve ese proveedor, y filtra la lista a medida que escribís, pero es una caja de texto normal: un modelo publicado esta mañana se escribí y ya está. Esas sugerencias salen de `/opt/boa/config/providers/<proveedor>.json`, que podés editar en el servidor; una actualización nunca pisa un archivo que hayas cambiado.
...
Si elegís un proveedor de nube, el campo desaparece: esa dirección es fija, la
...
Para uno de nube, lo más simple es la pestaña **Claves de API** de Ajustes, que luego usan todos los agentes de ese proveedor. Para darle a este agente en concreto una clave distinta, escribila en su propio home, como root:
...
`/opt/boa/config/keys/`; `--update` las mueve y elimina la carpeta vieja.
...
de los agentes, donde una copiá de seguridad descuidada las expondría todas a
la vez.

Para facturar un agente concreto a otra cuenta, poné una clave en el home de ese
...
- **Di qué hacer cuando se atasque.** Sin esto, los modelos se inventan una
  forma de rodear el obstáculo. Basta con «si no podés hacerlo, dilo en el
  tablero y para».
- **Dile que lea el tablero primero.** Es la única memoria que tiene entre
  ejecuciones.

Lo que no funciona: decirle que no use una herramienta. Si no querés que use
...
que se lleva de una ejecución a la siguiente. Escribí en él con `memory.append`
...
bueno, qué preferís vos. No el diario de lo que hizo: para eso está el tablero.

Podés leerla y corregirla en **Memoria**, dentro de la configuración del
agente. Un dato equivocado sobre el que el agente siga actuando merece
arreglarse a mano.

## Elegir herramientas

En **Herramientas**, marcá las que este agente puede usar. Una herramienta sin
...
- **`channel.write`** — mensajes para vos. Marcá también los canales en la pestaña **Canales**.
...
puede cambiarlos. Los cambiás vos; él no. Lo único suyo que puede editar es su
memoria.
- **`mail.*`** — el buzón configurado en **Ajustes → Correo**. Mirá más abajo.
- **`skill.read`** — los procedimientos escritos que estén marcados en el panel
  **Habilidades**. Hacen falta las dos cosas: la herramienta acá y al menos una
  habilidad allí.
...
Dale sólo `mail.read` a un agente que únicamente tenga que vigilar. Agregá
`mail.move` para archivar, y pensalo dos veces antes de dar `mail.delete` y
`mail.forward`.
...
un procedimiento que no sigue nadie. Las tuyas las escribís en el servidor, como
root:
...
el panel **Habilidades** del agente. Marcá una, dale al agente la herramienta
`skill.read` en **Herramientas** y apretá **Guardar**. Desde su siguiente ejecución el agente sabe que ese
...
- **El prompt es para lo que sirve un agente; una habilidad es cómo se hace un
  ...
  copias que se queden desactualizadas por separado.
...
- **Lo que un agente aprende por su cuenta muere con él.** Su memoria vive
  adentro de una carpeta que ningún otro agente puede abrir. Una habilidad es el
  lugar donde poner lo que querés que sepa también el siguiente agente.
...
El archivo empieza con una cabecera de dos líneas y después dice lo que tenga
que decir:
...
1. Volcá con `mysqldump --single-transaction`, nunca bloqueando las tablas.
2. Escribilo en /srv/backups/, nunca en /tmp.
...
```
...
Recargá la página del agente y la habilidad va a estar en la lista.
...
«ejecutá `dump.sh` de esta carpeta» en vez de detallar cuarenta líneas de shell.
...
decidió que lo necesita, y sólo entonces. Darle cinco habilidades a un agente le
cuesta un par de cientos de tokens por ejecución, no diez mil, que es por lo que
podés darle cinco sin pensar en la factura.

### Si borrás una habilidad que están usando varios agentes
...
- El nombre **sigue en el `info.json` del agente** hasta que algo lo reescriba,
  así que devolver la carpeta a su lugar restaura la habilidad sin hacer nada
  más.
- Si el agente la pide igual —puede acordarse del nombre de una ejecución
  anterior—, se le dice que la habilidad está en su lista y ya no está
  instalada, y que no se invente lo que decía.

Una cosa que conviene saber: **apretar Guardar en ese agente mientras falta la
habilidad la saca de la lista definitivamente.** La interfaz sólo guarda las
habilidades que existen, que es lo que impide que una borrada se quede ahí para
siempre. Devolvé la carpeta a su lugar antes de guardar el agente, o volvé a
...
La respuesta se recuerda, igual que con los puertos. Después, marcá las
...
el agente llama después a `image.send` con esa ruta. Activá **image.send** en
**Configuración del agente → Herramientas → Imágenes** y guardá. Los agentes
...
sesión para verlos. Podés enviar una captura existente pidiéndole al agente
...
Si elegís como secundario el **mismo proveedor** que el principal, el campo del
modelo se completa con un modelo *distinto* del catálogo de ese proveedor, no
...
Si aun así escribís la misma pareja a mano, una línea debajo del campo te lo
...
agente una vez más —sin herramientas y con un presupuesto chico aparte— la
respuesta que iba a dar, para que una ejecución que gastó todo su presupuesto
juntando datos no termine en «voy a revisar el sistema». El chat lo avisa
...
de 12 000 tokens terminó midiendo 24 901. El techo de tokens es, entonces, un
...
En **Cron**, escribí el crontab del agente. Es el crontab propio del
...
Si usás un proveedor de pago cuyo precio varía según la hora —DeepSeek cobra
...
que vos usás para ver qué ha pasado de verdad. Cada tarjeta lleva su historial:
...
tarde: no se despierta a nadie por ella, y su agente la va a encontrar en su
próxima ejecución programada. **Mover a** es cómo movés una tarjeta a mano:
este tablero no tiene arrastrar y soltar.

Tú podés añadir, mover y borrar cualquier tarjeta. **Un agente sólo ve y
...
Si le pedís que mueva una tarjeta que no es suya, le responden que no hay
...
que decir cuál de las dos. Escape cancela. Para seguir adelante hay que apretar
el botón rojo.
...
Abrí **Settings → Audio**. La configuración se guarda en el servidor y se aplica a las notas de voz y archivos de audio que llegan por Telegram.

1. Elegí **Local · whisper.cpp** o **API de un proveedor**. Inicialmente está desactivada.
2. En local, seleccioná un modelo y pulsá **Descargar modelo** si falta. El instalador deja listo `base`; la lista incluye los 30 modelos oficiales, los ingleses `.en`, los cuantizados y los modelos `ggml-*.bin` que instales manualmente en `/opt/boa/whisper/models/`. El tamaño y el estado se muestran en pantalla. Los modelos grandes necesitan más RAM y CPU.
3. Con una API, guardá antes su clave en **Claves API**. Aparecen OpenAI, Groq, Mistral, Together AI, Hugging Face y Cloudflare cuando tienen una clave. Elegí una sugerencia o escribí el identificador de otro modelo de transcripción del proveedor. Cloudflare ofrece sus dos variantes compatibles de Whisper y usa la credencial `account-id:api-token`.
4. Elegí el idioma (`auto` o un código como `es`), la duración máxima y si conservar el audio para reproducirlo; pulsá **Guardar**. Para probarlo, respondé por Telegram a un mensaje del agente con una nota de voz. También puedes seleccionar antes el agente con `/agents`.
...
Para actualizar una instalación existente, ejecutá como root el instalador de tu distribución con `--update`. Instalará FFmpeg, whisper.cpp y el modelo base bajo `/opt/boa/whisper/` sin activar la transcripción hasta que la configures.
...
igual que el tema. Un navegador que nunca eligió recibe lo más parecido a lo
que pide, e inglés si no hay nada parecido.
...
- **Ajustes → Agentes → Idioma en el que responden los agentes** agrega una
...
el servidor: describen cómo trabajás vos en esta máquina, y es razonable que el
móvil y el escritorio no opinen lo mismo.

## Mensajes emergentes

Cuando se guarda algo, la confirmación aparece en el centro del panel que
estás mirando y después desaparece sola. Está en el medio a propósito: el
botón Guardar del final de una pestaña larga escribía una línea arriba de
todo de la página, varias pantallas por encima de donde estabas mirando, así
que guardar parecía no hacer nada.
...
Los mensajes de error no le hacen caso a ese número. Se quedan hasta que los
cierres, con la «×» o con Escape, porque un error es el único mensaje que
tiene que seguir ahí cuando volvés a mirar la pantalla.

El color dice cuál de las tres cosas pasó:

| Color | Qué significa |
|---|---|
| Verde | Se guardó |
| Ámbar | **No hay cambios que guardar**: apretaste Guardar y no habías cambiado nada del formulario |
| Rojo | Falló. Dice qué salió mal y espera a que lo cierres |

El ámbar sale en todos los lugares donde podés apretar Guardar: en **Ajustes**
(la cuenta, el servidor de correo, las claves de API, los canales y las
preferencias del navegador) y en los ajustes de cada agente. Si apretás
Guardar dos veces, la segunda te lo dice, en vez de informar de un guardado
que no ocurrió. La excepción es un agente recién creado: su formulario tiene
los valores del ejemplo y nunca se guardó, así que Guardar los escribe y te
lleva a su chat.
...
| **Day High Contrast** | Fondo blanco, texto casi negro, bordes duros. Acá nada se rellena con el color de acento: el cuadro de cada agente en la barra lateral es un contorno, y tu propio mensaje va relleno de una tinta suavizada en vez de azul |
...
Definilas todas. Una variable que un tema se deje fuera conserva el valor de
...
tarjeta:
    ...
    Tenés una tarea asignada en una tarjeta:

    Id de la tarjeta: 1284
    Título: Renovar los certificados
    Descripción de la tarea: «Ejecutá el instalador con --update y avisá»
    A ejecutar: Inmediatamente

Si la tarjeta se la entregó otro agente, la primera línea dice quién: *manager
te asignó una tarjeta nueva*. Si la tarjeta estaba programada en vez de
...
que programás para esta noche y borrás esta tarde no deja nada en el chat,
...
tarjeta. Lo que dijiste antes en el chat se reenvía sólo cuando escribís vos un
...
Después, en la pestaña **Canales** de cada agente, marcá a qué canales puede escribir.
Necesita tanto `channel.write` como el canal en sí.

### Responderle a un agente desde Telegram
...
abra ese mismo bot se encuentra un chat vacío: ni comandos abajo de `/`, ni
descripción, ni nada para apretar salvo el botón Iniciar, que Telegram dibuja
en todos los bots y que ninguna API puede sacar.

Apretarlo no hace nada. El listener compara el `chat_id` de cada mensaje con
el que tenés configurado y descarta lo que no coincide, antes de ejecutar
ningún comando y antes de elegir agente.
...
persona, y guardarlo significaría que tu instalación va juntando en silencio
una lista de quién encontró el bot.
...
escribe en el registro en cada arranque, que es con lo que podés compararlo:
...
1. Abrí <https://discord.com/developers/applications> y apretá **New
   Application**. Ponele el nombre que quieras.
2. **Bot** en la barra lateral, después **Reset Token**, y copiá lo que te
   muestre. Ese es el `bot_token`, y se muestra una sola vez.
3. **OAuth2 → URL Generator**: marcá **bot**, y abajo **View Channels**,
   **Send Messages** y **Read Message History**. Abrí la URL que arma y
   agregá el bot a tu servidor.
4. En el propio Discord, activá **Ajustes → Avanzado → Modo desarrollador**,
   hacé clic derecho en el canal donde querés a los agentes y **Copiar ID del
...
En **Ajustes → Canales → Discord**, poné el token y el id del canal, marcá
**Permitir responder a los agentes desde Discord** y guardá. En unos segundos
...
Los mensajes escritos antes de prenderlo no se contestan: la primera pasada
sólo anota por dónde va el canal y arranca desde ahí.
...
`/agents` también sirve, si es lo que te salen escribir los dedos. Acá no hay
...
Las tres formas de dirigirse a alguien son las que ya conocés:

1. **Responder a algo que dijo un agente** va a ese agente, esté seleccionado
   quien esté. Clic derecho en su mensaje → Responder, o deslizalo en el
   celular.
2. **Nombrar a uno** —`@os-watcher mirá el disco`— le llega a él *y* lo deja
   seleccionado. `!os-watcher`, `@001` y `@News Miner qué hay de nuevo` sirven
   igual.
3. **Ninguna de las dos** va a quien elegiste la última vez.
...
mensajes como mucho. Podés responder a cualquiera de ellos y llega al mismo
agente. Si había más de lo que entra en cuatro, el último termina en `…`, y el
...
con Telegram, y vale un minuto: allá el bot compara el id del chat de cada
mensaje y descarta lo que no cuadra, así que un desconocido que encuentre tu
bot no consigue nada. Acá el bot lee un canal, y cualquiera que pueda publicar
en él puede lanzar ejecuciones en tu servidor.

Así que poné a los agentes en un **canal privado**, uno que veas sólo vos, o
vos y la gente de la que te fiás. El bot no necesita acceso a nada más.

### Si sólo querés avisos

Dejá el token vacío y poné una **URL de webhook**: **Ajustes del canal →
...
Los enlaces antiguos de `/tools/` redirigen acá y conservan la familia seleccionada.

Una herramienta cuya familia nadie nombró —un `.py` que alguien dejó en
...
un archivo que le podés dar a un generador de clientes.
...
## Dónde termina el informe de una ejecución

Una ejecución que lanzás desde el chat te contesta ahí. Una que arranca por una
tarjeta que vence contesta en el mismo lugar. Una que arranca por el **crontab
del propio agente** no contesta en ningún lugar concreto —nadie le preguntó
nada—, así que:

- **Siempre en Historial**, en **Últimas ejecuciones**: lo que dijo cada una y
  lo que costó. Es donde mirar cuando te preguntes qué estuvo haciendo un
  agente.
- **Y en el chat cuando merece interrumpirte**: la ejecución no terminó, o
  cambió algo que vos verías (una tarjeta, un mensaje a un canal, un buzón).
  Una ejecución que sólo miró se queda en el Historial. Si no, un agente con
...
llegó a arrancar”. Apretá **Ejecutar ahora** en un agente al que todavía no le
pusiste la clave de API, o mientras ese agente ya se está ejecutando, y la
...
escribía en un lugar que nadie lee. Una ejecución que no llegó a arrancar no
gasta una de las ejecuciones del día del agente, y tampoco cuenta como una
falla suya: no se ejecutó nada.
...
en una pestaña nueva. La cuenta con la que entraste no se muestra: una
instalación sólo tiene una, así que escribirla no te dice nada que no supieras.

## En qué idioma responden tus agentes

Escribí el prompt de cada agente en el idioma en el que pienses y te responderá
...
todos los agentes a la vez. Agregá UNA línea al prompt de sistema de cada uno,
...
Escribí el prompt de sistema en el idioma en el que pienses. Nada en el sistema
...
Reescribilo entero en tu idioma y esa regla se va con él: el agente sigue el
...
lugar: `--backup /mnt/usb/boa.tar.gz`. Dentro: las dos bases de datos,
...
**Tratalo con la misma privacidad que a la máquina.** Guarda todas las claves
...
crontabs y arranca todo. Entrá con el correo y la contraseña de la copia: los
dos se agregan a `/opt/boa/logs/install.log` bajo el encabezado `Credentials
...
**Elegí `--ports direct` y el HAProxy de la máquina ha desaparecido.** No
...
conservado como `haproxy.cfg.before-boa.<fecha>` si lo habías escrito vos. En
...
Para volver atrás, ejecutá el instalador con `--update --ports proxied`:
...
**`boa-proxy` se reinicia una y otra vez y nada escucha en el 11443.** Leé
...
pidió el proxy: normalmente, un contenedor chico. Una instalación hecha antes
de que esto se corrigiera todavía tiene `maxconn 2048` en
...
De ahí en más HAProxy se dimensiona con los descriptores que puede tener de
...
curl del pedido, si hay algo escuchando en el puerto, el estado de los seis
servicios y las últimas líneas de lo que imprimieron el proxy y la aplicación
web. `000` no es un código HTTP: es curl diciendo que no recibió ninguno. Un
servicio que se declara `started` junto a `nothing is listening on port 11443`
es un proceso que muere y vuelve a arrancarse cada pocos segundos, y su propio
log, unas líneas más abajo, dice por qué.

Una actualización que falló acá ya dejó puesta la versión anterior y la está
ejecutando. Una primera instalación no tiene a dónde volver, así que deja todo
en su lugar: arreglá lo que nombre el bloque y volvé a ejecutar el instalador
con `--update`.
...
**No pasa nada al pulsar Ejecutar ahora.** Mirá los dos puntos del final de la
...
**El agente se ejecuta pero no hace nada.** Mirá su panel de Historial. Luego
ejecutalo a mano y observa:
...
Agregá `--dry-run` para ver qué cargaría —proveedor, herramientas, techos— sin
llamar al modelo.

**Dice que no puede contactar con el modelo.** Con proveedores autoalojados,
comprobá que el servidor está en marcha y que la URL base es correcta. Con los
de nube, comprobá que el archivo de la clave existe en el home del agente y
que le pertenece.
