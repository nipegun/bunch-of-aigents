...
13. [Cuando algo no funciona](#cuando-algo-no-funciona)
...
- **Debian con systemd como PID 1.** Los seis servicios son unidades de
...
- **Alpine con OpenRC.** Los seis servicios son scripts de OpenRC supervisados
...
| `journalctl -u boa-web -f` | `tail -f /var/log/messages \| grep boa` |
...
| **backupwatcher** | Comprueba que las copias de seguridad se hicieron, son recientes y no son sospechosamente pequeñas. |
| **certwatcher** | Avisa cuando un certificado TLS está a punto de caducar, mientras aún da tiempo. |
| **diskcleaner** | Encuentra qué está llenando el disco y dice qué se podría quitar. Propone; nunca borra. |
| **logwatcher** | Lee los registros que le indiques y cuenta lo que es nuevo o lo que se ha vuelto frecuente. |
| **mailwatcher** | Vigila una cuenta de correo y actúa según las reglas que escribas en su prompt. |
| **newswatcher** | Lee los feeds RSS que le pongas en su prompt y cuenta lo que encaje con tus reglas. |
| **oswatcher** | Vigila esta máquina: disco, memoria, swap, carga y si los servicios están en marcha. |
| **sitewatcher** | Comprueba que los sitios que le des responden, responden rápido y siguen diciendo lo que decían. |
| **standup** | Lee el tablero cada mañana entre semana y escribe un resumen corto. |
| **webnavigator** | Maneja su propio navegador para hacer en un sitio lo que le pidas: buscar, entrar con tu sesión, llenar un formulario, ir pasando páginas y contarte lo que encontró. |

`webnavigator` es el único sin programación horaria: trabaja cuando se lo pedís
...
conviene dejarlo claro porque decide lo que un agente como `oswatcher` puede
hacer:
...
|---|---|
| **Sistema operativo** | Qué máquina es esta, cuánta memoria y disco quedan, y si los cuatro servicios están en marcha, cada uno en verde si está activo y en rojo si no lo está. Leído sin privilegios, igual que lo lee `oswatcher` |
| **Cuenta** | El único correo y la contraseña |
| **Correo** | SMTP, para cuando algo tenga que llegarte por email |
| **Canales** | Telegram, Discord, Mattermost, X: una caja para cada uno, con su propio Guardar |
...
- El **bot de Telegram** también lo habla: sus propias frases, el informe de
  `/status` y el texto de ayuda.
...
| **Night High Contrast** | Fondo casi negro, texto claro, bordes duros. Acá nada se rellena con el color de acento: el cuadro de cada agente en la barra lateral y el mensaje que enviaste son contornos, y tu propio mensaje va relleno de un blanco apagado en vez de azul |
...
| Discord | una URL de webhook de los ajustes del canal |
...
Telegram es el único canal que funciona también en el otro sentido. Marca
...
2. **Nombrarlo** —`@oswatcher revisa el disco`— va a él *y además* lo deja
...
**Herramientas**, en la barra lateral, es lo que hay instalado en el servidor,
a diferencia de lo que se le haya dado a un agente concreto. Cada familia es
una pestaña —Canales, Correo, Kanban, Memoria, Sistema operativo, Web— con el
...
(`/tools/?family=mail`), así que recargar o guardar el enlace la conserva.
