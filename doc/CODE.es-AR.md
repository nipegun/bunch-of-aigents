...
| `fRunPrivilegedCommand` | `backend/core/exec_daemon.py:90` | Ejecutá una lista de comando sin shell, opcionalmente como otro usuario |
...
| `fRunTool` | `backend/core/tool_registry.py:125` | Comprobá permisos y devuelve `(texto, es_error)`; una herramienta que lanza excepción nunca mata la ejecución |
...
| `fSelectInstalledSkills` | `backend/core/skills.py:176` | Los nombres de la lista de un agente que todavía existen en disco. **Todos los caminos hacia esta funcionalidad pasan por acá**, así que una habilidad borrada nunca llega a un prompt |
...
| `exec_daemon.fVerbWriteAgentInfo` | **A todos los campos de `info.json` que sobreviven a un guardado.** Reconstruye el archivo clave por clave, así que un campo que no nombre es un campo que la interfaz descarta en silencio la primera vez que alguien aprieta Guardar |
...
3. Agregá sus campos a `dChannelFields` en `frontend/static/js/settings.js`.
