...
privilegiado como root, que lee lo que le pongan delante. Medido sobre el
...
diario de todos, cada petición posterior de esa lista perdía un hilo más; un
...
que dijo `fstat`, porque el agente puede seguir añadiendo mientras se lee:
...
el único sitio que arranca un runner: rechaza un texto más largo que
...
cabe en el búfer nunca espera al hijo; este demonio no espera a los runners.
...
se le arrancó cuando rechaza, para que la conversación no se quede esperando
para siempre. `--prompt` y `--chat-message` se quedan, para una ejecución
arrancada a mano desde un terminal.
...
Donde systemd es PID 1 el demonio añade la segunda: `fStartRunner` mete la
...
porque un servicio supervisado que muere nada más arrancar lo da por arrancado
la propia orden que lo arrancó. Cuatro pruebas EJECUTAN las dos funciones:
contra un `curl` que se niega a conectar, contra un `ss` que ocupa el puerto y
otro que no, y contra un `rc-service` y un `systemctl` que contestan «stopped»
y devuelven un fallo.
...
`fRollBack` corría en un solo sitio: cuando fallaba ese `curl` final. Entre
...
acepta, una unidad que no se instala - y cada uno dejaba los servicios
...
de parar los servicios, y `fCleanup` - el trap de EXIT, que corre acabe como
acabe el proceso hijo - deshace el cambio cuando encuentra la marca puesta y
...
`fFinishUpdate`, porque una vez que la versión nueva ha respondido no queda
nada a lo que volver. Medido en las dos máquinas de pruebas con la clave
privada escondida: «Missing certificate files», «Putting the previous version
back», los siete servicios arriba y la página de inicio de sesión respondiendo
200 con el código anterior. Una prueba ejecuta los `fCleanup`, `fRollBack` y
...
que es la razón de que ninguna lo enseñara nunca, y `haproxy -c` aceptaba el
...
`fd-hard-limit 4000` acota lo que coge donde el límite es enorme. Medido con
...
publica bajo un límite duro de 4096 y exige que siga en pie, y lo arranca otra
vez con el viejo `maxconn 2048` y exige que muera: una prueba que sólo leía el
archivo es precisamente lo que dejó pasar esto.
...
- **Tres archivos se dejan fuera a propósito**: `config/ports.conf`,
  ...
  escribiendo en ese momento; sus líneas de credenciales se añaden al log
...
latencia de segundos y doce peticiones por minuto frente a un límite de
...
decisiones:
  ...
  - **Borrar mueve a la papelera** si la cuenta tiene una. Lo que un agente
    ...
    carpetas» se mantienen separados, porque sólo el primero puede acabar en
...
| `fReadAgentOwnedFile` | `backend/core/paths.py:392` | Cómo lee root un archivo del home de un agente: sobre el descriptor abierto, normal y del agente o rechazado, nunca más de un tope. El diario, el chat y la memoria pasan por aquí |
...
| `fRunPrivilegedCommand` | `backend/core/exec_daemon.py:118` | Ejecuta una lista de comando sin shell, opcionalmente como otro usuario |
| `fVerbCreateAgent` | `backend/core/exec_daemon.py:392` | Crea usuario, home, configuración y token; **deshace todo si algo falla** |
| `fStartRunner` | `backend/core/exec_daemon.py:300` | El único sitio que arranca un runner como el agente. El mensaje del chat o el prompt de la tarjeta van por su entrada estándar, nunca por la línea de órdenes |
...
| `fRunTool` | `backend/core/tool_registry.py:136` | Comprueba permisos y devuelve `(texto, es_error)`; una herramienta que lanza excepción nunca mata la ejecución |
...
| `fSelectInstalledSkills` | `backend/core/skills.py:194` | Los nombres de la lista de un agente que todavía existen en disco. **Todos los caminos hacia esta funcionalidad pasan por aquí**, así que una habilidad borrada nunca llega a un prompt |
...
| Un archivo que el demonio lee del home de un agente (`runs.jsonl`, `chat.jsonl`, `memory.md`) | Se lee sólo a través de `paths.fReadAgentOwnedFile`. Un archivo nuevo que el demonio lea de un home tiene que pasar también por ahí, o root lee lo que el agente le ponga delante: un FIFO que nunca contesta, un enlace a cualquier archivo de la máquina |
| Lo que guarda una copia de seguridad (la línea de `tar` de `fDoBackup`) | Un estado nuevo bajo `/opt/boa/` que sea de la instalación y no del código hay que añadirlo ahí, o una restauración en una máquina nueva lo pierde |
...
3. Añade sus campos a `dChannelFields` en `frontend/static/js/settings.js`, y
...
4. Escribe `fRead<Nombre>Messages` en `channels.py`, devolviendo los mensajes
   del más antiguo al más reciente.
5. Escribe `<nombre>_inbox.py`: dos tablas en `db.cAppSchema` y el agente
   seleccionado en un ajuste.
6. Escribe `<nombre>_listener.py` apoyado en `agent_routing`, que ya tiene la
   lista de agentes, la coincidencia por nombre, la respuesta cerrada y el
   informe de estado. Lo que queda es el protocolo.
7. Escribe `<nombre>_texts.py` con las frases que ese canal diga de otra
   manera, cayendo en `telegram_texts` para el resto.
8. Un archivo de servicio en `deploy/systemd/` y otro en `deploy/openrc/`, el
   nombre en `lServices` y en los siete sitios donde el instalador de Debian
...
5. Añade un bloque a `telegram_texts.dTexts` y su etiqueta al
   `lSupportedLanguages` de ese módulo, o el bot se queda en inglés. Añade
