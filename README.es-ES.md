...
- **Debian con systemd como PID 1, o Alpine con OpenRC.** El sistema de init
  no es un detalle aquí: los seis servicios son unidades de systemd en uno y
...
- **Herramientas ampliables.** Cada herramienta es un archivo `.py` en
  `/opt/boa/tools/`. Pon uno nuevo y aparece en la interfaz.
...
- **Responderles desde Telegram.** Elige un agente con /agents, o responde a un
  mensaje que te mandó uno, y lo que escribas le llega, arranca una ejecución y
  vuelve contestado a tu móvil. El intercambio entero está también en la
  conversación de ese agente en la interfaz web, las dos mitades, así que una
  sola pantalla sigue mostrándolo todo.
...
5. Crea un agente: `manager`, el orquestador. Todo lo demás son ejemplos que
   se eligen al pulsar **+**: `webnavigator`, `oswatcher`, `mailwatcher` y
