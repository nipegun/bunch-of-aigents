...
kanban compartido que puedes ver en el navegador, y pueden escribirte por
...
- **Debian con systemd como PID 1, o Alpine con OpenRC.** El sistema de init
  no es un detalle aquí: los siete servicios son unidades de systemd en uno y
  ...
  respuesta correcta pero una descarga tirada. Compruébalo con
  ...
  hace falta nada por adelantado: el instalador añade `openrc` él mismo si la
  máquina no lo tiene.
...
El instalador también compila **whisper.cpp v1.9.4** en `/opt/boa/whisper/`, instala FFmpeg y descarga el modelo multilingüe `base` (unos 142 MiB adicionales). La compilación, las dependencias del sistema y el navegador necesitan espacio adicional. Si falta Python, lo instala antes de comprobar su versión. En Alpine, los servicios liberan la salida SSH del instalador para que éste devuelva el control al terminar.

## Qué hace

- **Audios de Telegram.** En **Settings → Audio** eliges transcripción local con whisper.cpp o una API de OpenAI, Groq, Mistral, Together AI, Hugging Face o Cloudflare cuya clave ya esté guardada. Ofrece los 30 modelos nativos, con descarga bajo demanda. La nota de voz llega como texto al agente al que respondes y aparece en su chat web, con reproducción opcional del audio.
...
Eliges por agente cuáles recibe. Una herramienta que no se le da a un agente es
...
paso. Una carpeta cada una, dentro de `/opt/boa/skills/`:
...
Las tuyas las escribes por SSH, como root, y una actualización nunca las toca.
...
O descargarlo antes y leerlo antes de ejecutarlo:
...
4. Pregunta si servir la aplicación en 11080/11443 con un HAProxy delante en
   ...
   un `--update` no vuelve a preguntar ni la cambia en silencio. Elegir `direct` retira el
   HAProxy de la máquina: parado, fuera de todos los runlevels o enmascarado,
   y su `/etc/haproxy/haproxy.cfg` borrado si lo escribió este instalador o
   conservado como `haproxy.cfg.before-boa.<fecha>` si lo escribió otro, para
   que nada se quede con el 80 y el 443 en el siguiente arranque. El paquete
   haproxy se queda: el proxy de la propia aplicación es ese binario.
5. Crea un agente: `manager`, el orquestador. Todo lo demás son ejemplos que
   se eligen al pulsar **+**: `web-navigator`, `os-watcher`, `mail-watcher` y
...
6. Instala y arranca siete servicios: units de systemd en Debian, servicios
...
8. Le pide a la aplicación su página de inicio de sesión antes de decir que ha
   ...
   petición, si hay algo escuchando en el puerto, el estado de los seis
   servicios y las últimas líneas de lo que imprimieron— y nombra el log en
   vez de dar la instalación por buena. Un `--update`, además, deja antes la
   versión anterior funcionando otra vez.
...
```bash
./install-update-reinstall-debian.sh --backup                       # escribe /root/boa-backup-<fecha>.tar.gz
...
Las mismas dos flags en el script de Alpine. Una copia es un único archivo,
de `root` y con modo 0600, tomada con los servicios en marcha. Contiene todo
lo que la instalación es y el código no: las dos bases de datos, copiadas con
la API de copia de la propia SQLite para que no se pierda nada que siga en un
write-ahead log; las claves de los proveedores, los secretos de los canales y
el secreto de sesión; los certificados; cada agente con su home, sus archivos
protegidos, su usuario de Linux y su crontab; las skills y las herramientas
escritas en este servidor; y el log de instalación, por la contraseña que
guarda. No contiene el código, el entorno de Python ni el navegador, ni las
dos decisiones propias de esta máquina, el modo de puertos y si tiene
navegador, así que una restauración nunca importa las de otra máquina.

Para pasar a una máquina nueva: `--install` allí y después `--restore` con el
archivo. La restauración para los servicios, vuelve a crear los usuarios de
los agentes con los mismos nombres y, cuando están libres, los mismos ids,
sustituye los datos, instala los crontabs y arranca todo. El login pasa a ser
el de la copia, y su contraseña se añade a `/opt/boa/logs/install.log`, bajo
el encabezado `Credentials of the restored backup`. Pide confirmación salvo
que se pase `--yes`.

El archivo guarda todos los secretos de la instalación. Hay que tratarlo con
la misma privacidad que a la máquina.

## Un navegador para cada agente

Activa `image.send` para adjuntar las capturas PNG al chat web y a las respuestas de Telegram. Los **web-navigator** nuevos ya la incluyen; en los existentes se activa en su pestaña **Herramientas → Imágenes**.
...
Con esa configuración puesta, abre:
...
2. Pulsa **+** en la barra lateral para crear tu primer agente.
3. Elige su proveedor y su modelo. Para Ollama en la misma máquina, los valores
   por defecto ya son correctos.
4. Si es un proveedor de nube, escribe la clave de API en el home del agente:
   ```bash
...
En **Ajustes → Canales**, los canales aparecen en orden alfabético y cada canal
configurado muestra **Configurado** en verde, como una clave API guardada.
En **Sistema operativo**, los estados de los servicios se alinean con los valores de la máquina.

**Night High Contrast** rellena de gris el agente seleccionado y marca cada
pestaña seleccionada con un marco cerrado. **Cerrar sesión** pide confirmación
...
| `boa-telegram` | `boa` | Escucha en Telegram, para que puedas responder a un agente desde el móvil |
...
`supervise-daemon` y lo que imprime cada uno va a su propio archivo en
`/opt/boa/logs/`, rotado cada semana:
...
- **Ningún agente se ejecuta como root, nunca.** Sólo `boa-exec` lo hace, y
  acepta una lista cerrada de dieciocho operaciones por un socket Unix. Entre ellas
...
- [doc/MANUAL.es-ES.md](doc/MANUAL.es-ES.md) — cómo usarlo, en el día a día.
- [doc/CODE.es-ES.md](doc/CODE.es-ES.md) — cómo está construido, para
...
[README.es-AR.md](README.es-AR.md) (español de Argentina).
