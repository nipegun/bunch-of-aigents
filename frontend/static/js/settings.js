...
const lTabs = ["system", "account", "email", "keys", "channels", "agents",
...
/* Switches a channel can have, as opposed to values it is given. Telegram is
 * the only channel messages can arrive through, and listening is separate from
 * sending on purpose: an installation can have agents that write to Telegram
 * without anybody being able to write back to them. */
const dChannelSwitches = {
  telegram: [["listen", "settings.channelListen",
...
async function fLoadTab(pTab) {
  if (sLoadedTabs.has(pTab)) { return; }
...
function fDescribeTheme(dTheme) {
  /* The names and descriptions come from the theme files themselves, which are
   * in English and are not translated: a theme is somebody's palette with a
   * name, and translating "Nord" would help nobody find it. */
  if (!dTheme) { return ""; }
  const lParts = [];
  if (dTheme.scheme) { lParts.push(dTheme.scheme); }
  if (dTheme.description) { lParts.push(dTheme.description); }
  return lParts.join(" · ");
}
...
async function fRenderChannelForms() {
  ...
  (dResult.channels || []).forEach(function (dChannel) {
    ...
    vForm.appendChild(fCreateElement("p", "field-hint",
      dChannel.configured
    ...
    (dChannelFields[dChannel.name] || []).forEach(function (vFieldName) {
      ...
      /* Secrets never come back from the server, so the fields start empty and
       * an empty field means "leave it as it is". */
      vInput.type = vFieldName.indexOf("token") !== -1 ? "password" : "text";
      vInput.autocomplete = "off";
      vInput.placeholder = dChannel.configured ? "••••••••" : "";
      vField.appendChild(vInput);
      vForm.appendChild(vField);
    });
...
async function fSaveAccount(pEvent) {
  ...
  if (vPassword) { dPayload.password = vPassword; }
  try {
