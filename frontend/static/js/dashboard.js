...
function fRenderRunHistory(lEntries) {
  ...
  /* The NEWEST ones, newest first.
   *
   * The journal arrives oldest first - fReadEntries says so in its own
   * docstring - so `.slice(0, cShownRuns)` took the ten OLDEST runs and
   * pinned them there for ever. An agent with more than ten runs behind it
   * showed a History that never changed again.
   *
   * Measured in a real browser against a real installation: an agent whose
   * most recent run had just been refused for want of an API key showed a
   * provider failure from forty minutes earlier, and the refusal never
   * appeared at all. The heading over this list says "Last runs". */
  const lRuns = (lEntries || []).filter(function (dEntry) {
    return dEntry.kind === "run_finished";
  }).slice(-cShownRuns).reverse();
...
function fRenderUsage(dUsage) {
  ...
  const lRows = [
    ...
    /* Cleaned the same way the rows below it are: this cell showed
     * "2026-09-19T03:44:29Z" directly under a list of "2026-09-19 04:31:18". */
    [fTranslate("agents.usageLast", "Last run"),
     dUsage.last_run_at
       ? String(dUsage.last_run_at).replace("T", " ").replace("Z", "")
       : "—"],
    /* Through the same key as the History list, not printed raw: this row
     * showed the English word in the middle of a Spanish page, and the
     * screenshot that found it said "Último estado: refused". */
    [fTranslate("agents.usageStatus", "Last status"),
     dUsage.last_status
       ? fTranslate("agents.runStatus." + dUsage.last_status,
                    dUsage.last_status)
       : "—"],
  ];
...
const dChatSources = {
  ...
  discord: ["chat.sourceDiscord", "via Discord"],
};

function fRenderChatAttachments(pBubble, pMessage) {
  if (!Array.isArray(pMessage.attachments)) { return; }
  const vAgentId = fGetSelectedAgentId();
  if (!/^\d{3}$/.test(vAgentId)) { return; }
  pMessage.attachments.slice(0, 16).forEach(function (dImage) {
    if (!dImage || typeof dImage.id !== "string" || !/^[a-f0-9]{32}$/.test(dImage.id)) { return; }
    const vName = String(dImage.name || "image.png");
    const vFigure = fCreateElement("figure", "chat-attachment");
    const vLink = document.createElement("a");
    vLink.href = cApiBase + "/agents/" + encodeURIComponent(vAgentId)
      + "/attachments/" + encodeURIComponent(dImage.id);
    vLink.target = "_blank";
    vLink.rel = "noopener noreferrer";
    const vImage = document.createElement("img");
    vImage.alt = fTranslate("chat.image", "Image: {name}").replace("{name}", vName);
    vImage.loading = "lazy";
    vImage.decoding = "async";
    vImage.addEventListener("error", function () {
      vImage.hidden = true;
      vFigure.appendChild(fCreateElement("p", "field-hint",
        fTranslate("chat.imageUnavailable", "Could not load this image.")));
    }, { once: true });
    vImage.src = vLink.href;
    vLink.appendChild(vImage);
    vLink.appendChild(fCreateElement("span", "chat-attachment-name", vName));
    vFigure.appendChild(vLink);
    if (dImage.caption) {
      vFigure.appendChild(fCreateElement("figcaption", "chat-attachment-caption",
                                        String(dImage.caption)));
    }
    pBubble.appendChild(vFigure);
  });
}

function fRenderChat(lMessages, pPending, pDropped) {
  ...
  const vSignature = JSON.stringify(lMessages.map(function (dMessage) {
    return [dMessage.id, dMessage.pending, dMessage.attachments, dMessage.audio_id, dMessage.audio_available];
  })) + String(pPending) + String(pDropped || 0);
  ...
  /* The daemon trims a reply by bytes before sending it, because retention
   * allows 500 chat lines and caps the length of none of them. What it cut is
   * said here rather than left to look like the whole conversation: a history
   * that quietly begins in the middle is one nobody can tell from a history
   * that begins there. */
  if (pDropped) {
    vLog.appendChild(fCreateElement(
      "p", "chat-trimmed",
      fTranslate("chat.olderNotShown",
                 "{count} older message(s) are not shown: this conversation " +
                 "is longer than one reply can carry.")
        .replace("{count}", pDropped)));
  }
  ...
  lMessages.forEach(function (dMessage) {
    ...
    if (/^[a-f0-9]{32}$/.test(dMessage.audio_id || "")) {
      vBubble.appendChild(fCreateElement("div", "chat-audio-heading",
        fTranslate("audio.transcript", "Audio transcript")));
      if (dMessage.audio_available) {
        const vAudio = document.createElement("audio");
        vAudio.controls = true;
        vAudio.preload = "none";
        vAudio.className = "chat-audio";
        vAudio.src = "/api/admin/agents/" + encodeURIComponent(fGetSelectedAgentId())
          + "/audio/" + encodeURIComponent(dMessage.audio_id);
        vBubble.appendChild(vAudio);
      }
    }
    ...
    fRenderChatAttachments(vBubble, dMessage);
...
async function fRefreshChat(pAgentId) {
  try {
    ...
    fRenderChat(dResult.messages || [], dResult.pending, dResult.dropped);
