...
def fAppendRunMessage(pAgentId, pText, pReason, pToolsUsed=None):
...
def fAppendCardMessage(pAgentId, dCard, pTurnId=""):
  ...
  return fAppendMessage(
    pAgentId, cRoleCard,
    str(dCard.get("body") or "")[:cMaxCardBodyLength],
    pTurnId, pPending=True,
    pMetadata={
      "card_id": dCard.get("id"),
      "card_title": str(dCard.get("title") or "")[:cMaxCardTitleLength],
      "card_run_at": str(dCard.get("run_at") or ""),
      "card_immediate": bool(dCard.get("immediate")),
      "card_assigned_by": str(dCard.get("assigned_by") or ""),
      "card_assigned_by_name": str(dCard.get("assigned_by_name") or ""),
    }
  )


def fBuildCardTurnText(dMessage):
  ...
  card since.
  """
  vTitle = str(dMessage.get("card_title") or "")
  vBody = str(dMessage.get("text") or "")
  vHeader = "Card #%s: %s" % (dMessage.get("card_id"), vTitle)
...
def fReadMessages(pAgentId, pLimit=None):
  ...
  lMessages = []
  try:
    with open(fGetChatPath(pAgentId), "r", encoding="utf-8") as vFile:
      for vLine in vFile:
        vLine = vLine.strip()
        if not vLine:
          continue
