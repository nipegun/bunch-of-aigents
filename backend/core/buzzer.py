...
def fBuildPrompt(pdCard):
  """Return what the agent is told when this card wakes it."""
  vTemplate = (cManagerPromptTemplate
               if str(pdCard.get("owner_agent")) == cManagerAgentId
               else cCardPromptTemplate)
  return vTemplate % {
    "id": pdCard["id"],
    "title": pdCard["title"],
    "body": (pdCard.get("body") or "").strip() or "(no instructions on the card)",
  }
...
def fBuildCardAnnouncement(pdCard):
  ...
  the column.
  """
  vAssignedBy = str(pdCard.get("assigned_by") or pdCard.get("created_by") or "")
  return {
    "id": pdCard.get("id"),
    "title": pdCard.get("title") or "",
    "body": pdCard.get("body") or "",
    "run_at": pdCard.get("run_at") or "",
    "immediate": kanban.fWasRequestedImmediately(pdCard),
...
def fRingOne(pdCard):
  ...
  """
  vAgentId = str(pdCard.get("owner_agent") or "")
  try:
    dResult = exec_client.fRunNow(
      vAgentId, pPrompt=fBuildPrompt(pdCard), pOnlyIfIdle=True,
      pCard=fBuildCardAnnouncement(pdCard)
    )
  except exec_client.ExecError as vError:
    # The executor is down or restarting. The card keeps its turn.
    fLogLine("Card #%s: cannot reach the executor: %s" % (pdCard["id"], vError))
    return False
  ...
  kanban.fMarkCardBuzzed(
    pdCard["id"], "woke agent %s for this card" % (vAgentId,)
  )
  fLogLine("Card #%s: started a run for agent %s." % (pdCard["id"], vAgentId))
  return True
