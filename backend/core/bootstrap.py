...
def fStoreAdministrator(pEmail, pPassword):
  ...
  The session epoch carries over and goes up by one rather than starting again
  at zero. Deleting the row and inserting a fresh one would reset it, and a
  reset epoch is a session anybody still holding a cookie from the previous
  account could walk back in with: their cookie says 0, and the brand new row
  would say 0 too. Writing a new password has to end every session that
  existed under the old one, whoever wrote it.
  """
  vHash = fHashPassword(pPassword)
  vConnection = db.fOpenAppDb()
  try:
    vRow = vConnection.execute(
      "SELECT session_epoch FROM admin WHERE id = 1"
    ).fetchone()
    try:
      vEpoch = int((vRow["session_epoch"] if vRow else 0) or 0)
    except (TypeError, ValueError, IndexError, KeyError):
      vEpoch = 0

    vConnection.execute("DELETE FROM admin")
    vConnection.execute(
      "INSERT INTO admin (id, email, password_hash, session_epoch) "
      "VALUES (1, ?, ?, ?)",
      (pEmail, vHash, vEpoch + 1),
    )
    vConnection.commit()
  finally:
    vConnection.close()
