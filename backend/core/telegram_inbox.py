...
Three small tables in the application database, written and read by
...
the listener when somebody answers.
  ...
  telegram_deliveries: the text and images already delivered for a pending
                      turn, so a retry resumes with the missing part.
...
None of these tables is history. The first is pruned to the last few hundred rows -
nobody replies to a message from last month - and the second empties itself as
runs finish. Removing a pending turn also removes its delivery records.
"""
...
def fListDeliveredParts(pTurnId):
  """Return the text/image parts already delivered for this pending turn."""
  vConnection = db.fOpenAppDb()
  try:
    lRows = vConnection.execute(
      "SELECT part FROM telegram_deliveries WHERE turn_id = ?",
      (str(pTurnId),)).fetchall()
    return {vRow["part"] for vRow in lRows}
  finally:
    vConnection.close()


def fRememberDeliveredPart(pTurnId, pPart):
  vConnection = db.fOpenAppDb()
  try:
    vConnection.execute(
      "INSERT OR IGNORE INTO telegram_deliveries (turn_id, part) VALUES (?, ?)",
      (str(pTurnId), str(pPart)))
    vConnection.commit()
  finally:
    vConnection.close()
...
def fIsExpired(pdPending, pNow=None):
  ...
  try:
    ...
    return True
  # SQLite timestamps are UTC. mktime interprets them as local time and its
  # daylight-saving flag can make a fresh question look an hour old.
