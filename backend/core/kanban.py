...
def fListCards(pState=None, pOwnerAgent=None, pLimit=None, pForAgentId=None):
  """Return cards, newest first, optionally filtered by column and owner.

  `pForAgentId` narrows to the cards one agent may know about - the ones it
  owns and the ones it created - and it does so in the WHERE clause, BEFORE
  the LIMIT.

  That ordering is the whole point. Filtering afterwards meant taking the
  newest 50 rows of the whole board and then keeping whichever of them
  happened to belong to this agent: one card of its own behind 51 of somebody
  else's came back as an empty board, and the agent left work undone because
  it could not see a card that was sitting there.
  """
  lConditions = []
  lParameters = []
  if pState:
  ...
  if pForAgentId:
    # The same two columns fAgentOwnsCard reads, so that the SQL and the
    # Python answer cannot drift apart into two different ideas of ownership.
    lConditions.append("(owner_agent = ? OR created_by = ?)")
    lParameters.append(str(pForAgentId))
    lParameters.append(str(pForAgentId))
...
def fGetBoard(pLimitPerState=None, pForAgentId=None):
  ...
  for vState in lStates:
    # Narrowed in the query rather than afterwards: see fListCards. Taking the
    # newest N and then keeping this agent's meant an agent whose card sat
    # behind N of somebody else's got an empty board.
    dBoard[vState] = fListCards(vState, None, pLimitPerState,
                                pForAgentId=pForAgentId)
  return dBoard
