...
def fBuildTable(plLines, pStart):
  ...
  lRows = []
  vIndex = pStart
  while vIndex < len(plLines) and cTableRowPattern.match(plLines[vIndex]):
    if not cTableDividerPattern.match(plLines[vIndex]):
      lRows.append(fSplitTableRow(plLines[vIndex]))
    vIndex += 1
...
def fBuildFencedCode(plLines, pStart):
  """Return (html, next index) for a fenced code block."""
  vLanguage = cFencePattern.match(plLines[pStart]).group(1).strip()
  lBody = []
  vIndex = pStart + 1
  while vIndex < len(plLines) and not cFencePattern.match(plLines[vIndex]):
    lBody.append(plLines[vIndex])
    vIndex += 1
...
def fBuildQuote(plLines, pStart):
  """Return (html, next index) for a run of quoted lines."""
  lBody = []
  vIndex = pStart
  while vIndex < len(plLines) and cQuotePattern.match(plLines[vIndex]):
    lBody.append(cQuotePattern.match(plLines[vIndex]).group(1))
    vIndex += 1
  return ("<blockquote>%s</blockquote>"
          % ("\n".join(fRenderInline(vLine) for vLine in lBody),), vIndex)


def fBuildList(plLines, pStart, pOrdered):
  ...
  lItems = []
  vIndex = pStart
  while vIndex < len(plLines):
    vMatch = cPattern.match(plLines[vIndex])
    if vMatch is None:
      break
    if pOrdered:
      lItems.append("%s. %s"
...
def fBuildParagraph(plLines, pStart):
  """Return (html, next index) for a run of ordinary lines."""
  lBody = []
  vIndex = pStart
  while vIndex < len(plLines):
    vLine = plLines[vIndex]
    if not vLine.strip():
      break
