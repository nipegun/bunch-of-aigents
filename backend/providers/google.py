...
class GoogleProvider(base.BaseProvider):
  ...
  # A Flash, for the same reason the previous default was the Flash of its own
  # generation: fast, cheap, and enough for tool calls. The newest one,
  # gemini-3.7-flash, is deliberately not it: measured against a real key it
  # answers 503 "This model is currently experiencing high demand", and a
  # default that fails on the first run is worse than a default one version
  # behind. It costs the same and is one line away in the LLM tab.
  cDefaultModel = "gemini-3.6-flash"
