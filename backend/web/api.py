...
def fFailure(pMessage, pStatusCode=400):
  """Return a failed JSON response.

  The message is meant to be shown to the user as it is, so it says what to do
  rather than naming an exception class.
  """
  return jsonify({"ok": False, "error": str(pMessage)}), pStatusCode
...
def fCreateAgent():
  ...
  try:
    dResult = exec_client.fCreateAgent(
      ...
      # two copies of oswatcher have to be able to coexist.
...
def fUpdateAgent(vAgentId):
  ...
  try:
    if "info" in dBody:
      exec_client.fWriteAgentInfo(vAgentId, dBody["info"])
    if "system_prompt" in dBody:
      exec_client.fWriteSystemPrompt(vAgentId, dBody["system_prompt"])
    if "crontab" in dBody:
      exec_client.fWriteCrontab(vAgentId, dBody["crontab"])
    if "memory" in dBody:
      exec_client.fWriteMemory(vAgentId, dBody["memory"])
    dInfo = exec_client.fReadAgentInfo(vAgentId).get("info") or {}
  except exec_client.ExecError as vError:
    return fFailure(vError)
  return fSuccess({"info": dInfo})
...
def fUpdateSettings():
  ...
  dSettings = dBody.get("settings") or {}
...
def fGetStatus():
  ...
  dStatus["agent_api"] = os.path.exists(agent_api.cAgentSocketPath)
