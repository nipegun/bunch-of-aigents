...
- **Debian con systemd como PID 1, o Alpine con OpenRC.** El sistema de init
  no es un detalle acá: los siete servicios son unidades de systemd en uno y
...
- **Herramientas ampliables.** Cada herramienta es un archivo `.py` en
  `/opt/boa/tools/`. Poné uno nuevo y aparece en **Ajustes → Herramientas**,
...
- **Responderles desde Telegram o Discord.** Elegí un agente con /agents, o
  respondé a un mensaje que te mandó uno, y lo que escribas le llega, arranca
  una ejecución y vuelve contestado a tu celular. El intercambio entero está
  también en la conversación de ese agente en la interfaz web, las dos
  mitades, así que una sola pantalla lo sigue mostrando todo.
...
| `image.send` | Adjuntar un PNG al chat web y a la respuesta de Telegram cuando la conversación empezó ahí. La imagen ocupa todo el ancho interior del mensaje web |
...
5. Crea un agente: `manager`, el orquestador. Todo lo demás son ejemplos que
   se eligen al apretar **+**: `web-navigator`, `os-watcher`, `mail-watcher` y
...
8. Le pide a la aplicación su página de inicio de sesión antes de decir que
   terminó. Si esa página no llega, lo dice, escribe en ese mismo log el
   aspecto que tenía la máquina en ese momento —qué entendió curl del pedido,
   si hay algo escuchando en el puerto, el estado de los siete servicios y las
   últimas líneas de lo que imprimieron— y nombra el log en vez de dar la
   instalación por buena. Un `--update`, además, deja antes la versión
   anterior funcionando de nuevo.
...
el de la copia, y su contraseña se agrega a `/opt/boa/logs/install.log`, bajo
...
Activá `image.send` para adjuntar las capturas PNG al chat web y a las respuestas de Telegram. Los **web-navigator** nuevos ya la incluyen; en los existentes se activa en su pestaña **Herramientas → Imágenes**.
...
- **`check port 11080`**, para que el health check llame al puerto HTTP plano
  ...
  son de un mismo proceso, así que si uno responde el otro está. No agregues
